 ...
 function calcRoute(){
 	...
 	serviceDirection.route(request, (result, status) => {
 		if (status == google.maps.DirectionsStatus.OK) {
 			...
 			output_result.innerHTML = "<b>From:</b> " + document.getElementById("from").value + ".<br /><b>To:</b> " + document.getElementById("to").value + ".<br /><b>Great job, you saved " + result.routes[0].legs[0].distance.text + "s of driving to " + document.getElementById("to").value + "</b>.<br /><b>The Duration is </b><i class='fa fa-stopwatch'></i> : " + result.routes[0].legs[0].duration.text + ".";
